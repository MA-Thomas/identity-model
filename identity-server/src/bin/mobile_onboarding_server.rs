...
mod server {
    ...
    use identity_adapters::{device::*, oidc::*};
    ...
    type AccountRuntime = PostgresEncryptedMobileOnboardingRuntime<
        ...
        RingAes256GcmFactEncryptor<identity_adapters::codec::IdentityPlaintextCodec>,
    ...
    fn build_runtime(
        ...
        let account_repository = EncryptedWorkflowService::new(
            storage.clone(),
            ...
            RingAes256GcmFactEncryptor::with_codec(
                identity_adapters::codec::IdentityPlaintextCodec,
            ),
            key.clone(),
            config.materialization_policy_refs.clone(),
        );
        let identity_repository = EncryptedWorkflowService::new(
            storage,
            Aes256GcmFactEncryptionMetadataPlanner::new(
                config.fact_key_id.clone(),
                config.fact_nonce_domain,
    ...
    fn serve(
        ...
        let workers: Vec<std::thread::JoinHandle<()>> = (0..worker_threads)
            .map(|index| {
                ...
                std::thread::Builder::new()
                    ...
                    .map_err(|error| format!("could not spawn HTTP worker: {error}"))
            })
            .collect::<Result<_, _>>()?;
        ...
        for worker in workers {
            while !worker.is_finished() {
                if Instant::now() >= drain_deadline {
                    return Err("shutdown grace period elapsed with HTTP workers still busy".into());
                }
        ...
        if worker_failed {
            return Err("HTTP worker exited unexpectedly".into());
        }
