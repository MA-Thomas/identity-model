...
impl From<WorkflowAppendError<PostgresAdapterError>> for MobileIdentityOnboardingCommandError {
    fn from(error: WorkflowAppendError<PostgresAdapterError>) -> Self {
        match error {
            WorkflowAppendError::Encryption(error) => Self::Encryption(error),
            WorkflowAppendError::Storage(PostgresAdapterError::Repository(error)) => {
                Self::Repository(error)
            }
            WorkflowAppendError::Storage(error) => Self::Storage(error),
        }
    }
}
impl From<WorkflowReadError<PostgresAdapterError>> for MobileIdentityOnboardingCommandError {
    fn from(error: WorkflowReadError<PostgresAdapterError>) -> Self {
        match error {
            WorkflowReadError::Storage(error) => Self::Storage(error),
            WorkflowReadError::Materialization(error) => Self::Materialization(error),
        }
    }
}
...
impl From<WorkflowAppendError<PostgresAdapterError>> for MobileOnboardingCommandError {
    fn from(error: WorkflowAppendError<PostgresAdapterError>) -> Self {
        match error {
            WorkflowAppendError::Encryption(error) => Self::Encryption(error),
            WorkflowAppendError::Storage(PostgresAdapterError::Repository(error)) => {
...
impl From<WorkflowReadError<PostgresAdapterError>> for MobileOnboardingCommandError {
    fn from(error: WorkflowReadError<PostgresAdapterError>) -> Self {
        match error {
            WorkflowReadError::Storage(error) => Self::Storage(error),
            WorkflowReadError::Materialization(error) => Self::Materialization(error),
    ...
    E: FactPayloadEncryptor,
{
    ...
    let replayed_projection = project_identity_history(
        composition.subject_id.clone(),
        &composition
            .child_slices
            .iter()
            .flat_map(|slice| slice.facts.clone())
            .collect::<Vec<_>>(),
    );
...
pub async fn execute_postgres_encrypted_mobile_identity_onboarding_command<M, E>(
    ...
    continuity_provider: &impl ContinuityVaultProvider,
    id_generator: &mut impl IdGenerator,
    encrypted_repository: &mut EncryptedWorkflowService<SqlxPostgresEncryptedFactRepository, M, E>,
    persistence_context: MobileOnboardingEncryptedPersistenceContext,
) -> Result<MobileIdentityOnboardingCommandOutcome, MobileIdentityOnboardingCommandError>
where
    M: FactEncryptionMetadataPlanner,
...
fn build_verified_mobile_identity_onboarding_composition(
    ...
    let account_bootstrap = service.compose_account_session(
...
where
    ...
    E: FactPayloadEncryptor,
{
    let (client_context, bootstrap, _) = build_verified_mobile_onboarding_bootstrap(
        service,
        request,
        oidc_verifier,
        app_attest_verifier,
    ...
    let replayed_projection = bootstrap.workflow.projection.clone();
...
fn mobile_onboarding_outcome_from_bootstrap(
    ...
    replayed_projection: IdentityHistory,
